            ...
            >
                ...
                <div class="space-y-2">
                    <div class="flex flex-col text-xs font-medium">
                        <label for="email-address" class="text-left py-1">Email address</label>
                        <input
                    ...
                    <div class="flex flex-col text-xs font-medium">
                        <label for="password" class="text-left py-1">Confirm password</label>
                        <input
                            v-model="confirmPassword"
                            type="password"
                            placeholder="Confirm password"
                            required="true"
                            v-validity="payload.password !== confirmPassword ? 'Passwords do not match' : ''"
                            class="appearance-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
                        />
                    </div>
                </div>
...
const onSubmit = () => {
    if (!payload.password || !payload.email || !confirmPassword.value || payload.password !== confirmPassword.value) return
    console.log(payload)
